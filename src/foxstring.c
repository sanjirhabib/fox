...
char* chop_first(char* str, char* substr){
	if(!str_start(str,substr)){ return str; };
	return sub_str(str,str_len(substr),-2147483648);
};
char* chop_last(char* str, char* substr){
	if(!str_end(str,substr)){ return str; };
	return sub_str(str,0,-str_len(substr));
};
...
char* next_word(char* in,char* terminators){
	while(*in && str_chr(terminators,*in)){ in++; };
	char* head=in;
	while(*in && !str_chr(terminators,*in)){ in++; };
