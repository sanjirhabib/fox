...
int utest(char* found,char* expect,char* in,char* title){
	...
	if(!str_len(expect) && !str_len(found)){
		return 0; };
	if(str_eq(expect,found)){
		return 0; };
	if(str_len(expect)!=str_len(found)){
		px(xstr("len diffs expect: ",int_str( str_len(expect)), " != found: ",int_str( str_len(found)), End),1);
	}else{
		px("str ! eq",1); };
	px(xstr("\n", 
	"TEST: ", title, "\n", 
...
int ret_print(void* ret){
	if(!_printed){
		if(is_map(ret)){ px(json(ret,1),1); }
		else if(is_blob(ret)){ print(ret,stdout); }
