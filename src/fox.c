...
int compile(map* files,char* outdir,char* outfile,char* options,int release,int exe){
	map* out_c=build(files,outdir);
	if(str_eq(outfile,"fox")){
		px("mv fox .fox",1);
		if(is_file("fox") && exec("mv fox .fox",NULL)){
			fox_error("mv fox failed",0); }; };
	char* output=NULL;
	char* infiles=map_join(out_c," ");
	char* optimize="-g -O0";
	if(release){ optimize="-O3"; };
	int ret=exec(
		px(
		xstr("gcc ", optimize, " ", infiles, " ", file_rename("callfunc.c",outdir,NULL,NULL,NULL,NULL), " extern.c -o ", outfile, " -rdynamic -std=gnu99 -Wno-logical-op-parentheses -lfox -lm -I ", outdir, "/ ", options, " 2>&1", End),1),NULL);
	if(ret && str_eq(outfile,"fox")){
		if(is_file(".fox")){ exec("mv .fox fox",NULL); }; };
	if(ret){ xexit(-1); };
	return 0;
};
...
char* file_rename(char* file,char* dir,char* delext,char* addext,char* prefix,char* postfix){
	...
	char* ret=file;
	if(olddir){ ret=sub_str(ret,str_len(olddir),0); };
...
map* build(map* files,char* outdir){
	...
	for(int next1=next(files,-1,NULL,NULL); has_id(files,next1); next1++){ void* v2=map_id(files,next1);
		char* outfile=NULL;
		if(str_end(v2,".fox")){ outfile=file_rename(v2,outdir,".fox",".c",NULL,NULL); }
		else if(str_end(v2,".c.in")){ outfile=file_rename(v2,outdir,".in",NULL,NULL,NULL); };
		vec_add(ret,outfile);
		write_c(v2,outfile); };
...
char* int_human(int i,char* unit,char* zero){
	...
	while(addat<str_len(ret)){
		ret=xstr(sub_str(ret,0,-addat),",",sub_str(ret,-addat,0), End);
		addat+=4; };
...
char* str_add(char* str1,char* str2){ return xstr(str_dup(str1),str2, End); };
...
char* str_trim(char* str,char* chars){
	...
	int r=rchars(str,chars);
	if(!l && !r){ return str; };
	return substr(str,l,str_len(str)-l-r);
};
...
void* marked_str(char* str,char* name){
	...
	if(str[0]=='\\'){ return sub_str(str,1,0); };
	if(str[0]==':'){
		str++;
...
map* map_add_pair(map* mp,void* name,void* value,int type){
	...
	if(name && value && *(char*)value==':'){
		if(!map_len(mp)){
			return xadd(mp,"type",sub_str(value,1,0),"name",name, End);
		}else{
			char* str=value;
			str++;
			char* thealpha=read_alpha(&str);
...
map* add_name_val(map* mp,char* str,char** name,char** val,int type){
	...
	*val=NULL;
	void* v=NULL;
	if(type==Vector){
		char* s=str_trim(substr(name1,0,str-name1)," \t\n\r");
	...
	if(val1){
		...
		v=marked_str(substr(val1,0,str-val1),name1);
	}else if(name1){ v=marked_str(substr(name1,0,str-name1),NULL); name1=NULL; };
...
char* read_as_block(char** from,int level,char* terminator){
	...
	while(*str){
		...
		if(tabs>=level||line_isempty(str)){
			ret=xcat(ret,len-level>0 ? substr(str,level,len-level) : "\n", End);
		}else {break;};
		str+=len; };
	if(str_eq(str_trim(substr(str,str_level(str),line_len(str)-str_level(str))," \t\n\r"),terminator)){
...
map* str_split(char* str,char* by,int limit){
	...
	while((end=str_has(head,by))){
		vec_add(ret,substr(head,0,end-head));
		head=end+str_len(by);
...
char* read_upto_word(char** in,char* upto){
	...
	if(*str){
		*in=str+strlen(upto)-1;
		return substr(from,0,str-from+strlen(upto));
	}else{
		*in=str-1;
		return substr(from,0,str-from);
	};};
...
char* read_heredoc(char** in){
	char* str=*in;
	int len=line_len(str);
	char* hd=str_trim(substr(str,0,len)," \t\n\r");
	char* from=*in;
	int lno=0;
	str+=len;
	int tabs=str_level(str);
	...
	*in=str;
	return substr(from,0,str-from+1);
};
char* read_multistr(char** in){
	...
	while(*str){
		if(*str=='\\'){ if(!hd) {str++;}; }
			...
			if(!lno && word_start){
				hd=substr(word_start,0,str-word_start); };
			lno++;
			word_start=NULL;
		}else if(*str!=' ' && *str!='\t'){
			if(!word_start){
...
map* c_tokenizer(char** line,char term){
	...
	while(*str && *str!=term){
		...
		}else if(','==*str){ add_ctok(substr(str,0,1),mp,1); }
		else if(';'==*str){ add_ctok(substr(str,0,1),mp,1); };
		if(*str){ str++; }; };
...
map* add_semicolon(map* mp,int recurse){
	if(!mp) {return mp;};
	for(int idx=1;idx<mp->len;idx+=2){
		if(recurse && str_eq(map_id(mp,idx),"{") && (str_eq(map_id(mp,idx-2),")") || is_keyword(map_id(mp,idx-2)))){
...
map* xstr_parts(char* in){
	...
	int incode=0;
	char* str=in;
	if(strchr("\"'",*str)){ str=substr(str,1,-1); };//.str_unquote()
	while(1){
		if(incode){
			if(incode>1 && *str=='('){
				incode++;
...
map* colon_str(map* mp){
	for(int idx=1;idx<=map_len(mp);idx+=2){
		...
		if(str_len(str)>1 && *str==':'){
			str=str_quote(str_unquote(sub_str(str,1,0)));
			set(mp,idx,str); };
		if(idx>=2 && *str=='"' && str_eq(map_id(mp,idx-2),".")){
			vec_splice(mp,idx,1,xvec("map_val",NULL,"(",NULL,xvec(NULL,str, End),NULL,")", End)); }; };
	return mp;
...
int stm_end(map* mp,int idx,char* words,int including){
	...
	int from=idx;
	while(idx<mp->len){
		if(is_word(map_id(mp,idx),words)){
			if(including){
				while(is_word(map_id(mp,idx),words)){ idx+=2; }; };
			break; };
		idx+=2; };
	if(idx>from){ idx-=2; };
	return idx;
...
map* syn_func(map* syn,int with_body){
	...
	char* name=map_id(syn,len-params_at-4);
...
char* drop_right(char* str,char* w){
	if(w && is_str(w) && str_end(w,w)){ return sub_str(str,-str_len(w),0); };	
	return str;
};
char* drop_left(char* str,char* w){
	if(str && is_str(str) && str_start(str,w)){ return sub_str(str,str_len(w),0); };	
	return str;
};
...
char* file_ast(char* in){
	map* toks=x_toks(fox_read_file(in,1),0);
	write_file(toks_c(toks),"good.txt",0);
	void* ret=dx(block_ast(toks,";"),NULL,0);
	write_file(ast_c(ret),"bad.txt",0);
	return ret;
};
char* ast_c(map* ast){
	if(!ast){ return NULL; };
	if(is_str(ast)){ return (char*)ast; };
	if(is_vec(ast)){
		char* ret=NULL;
		for(int i=next(ast,-1,NULL,NULL); has_id(ast,i); i++){ void* v=map_id(ast,i);
			ret=xcat(ret,ast_c(v), End); };
//		if ret.str_len() && ast[0].tag==="," => ret=ret.sub_str(0,ret.str_len()-ast[0].tag.str_len())
		return ret; };
	if(is_word(map_val(ast,"tag"),"TNumber TString TName TChar")){
		return xstr(ast_c(map_val(ast,"left")), map_val(ast,"space"), map_val(ast,"right"), map_val(ast,"tail_space"), End);
	}else if(str_eq(map_val(ast,"tag"),"TCall")){
		return xstr(map_val(ast,"left"), map_val(ast,"space"), "(", map_val(ast,"left_space"), ast_c(map_val(ast,"right")), map_val(ast,"right_space"), ")", map_val(ast,"tail_space"), End);
	}else if(is_word(map_val(ast,"tag"),"( [ {")){
		return xstr(ast_c(map_val(ast,"left")), map_id(map_val(ast,"space"),0), map_val(ast,"tag"), map_id(map_val(ast,"space"),1), ast_c(map_val(ast,"right")), map_id(map_val(ast,"space"),2), char_str(closing_paren(fox_at(map_val(ast,"tag"),0))), map_val(ast,"tail_space"), End);
	}else if(str_eq(map_val(ast,"tag"),"if")){
		char* ret=xstr(map_id(map_val(ast,"space"),0), "if", map_id(map_val(ast,"space"),1), "(", map_id(map_val(ast,"space"),2), ast_c(map_id(map_val(ast,"left"),0)), map_id(map_val(ast,"space"),3), ")", map_id(map_val(ast,"space"),4), "{", map_id(map_val(ast,"space"),5), ast_c(map_id(map_val(ast,"right"),0)), map_id(map_val(ast,"space"),6), "}", End);
		map* map_1=map_val(ast,"left"); for(int i=next(map_1,-1,NULL,NULL); has_id(map_1,i); i++){ void* v=map_id(map_1,i);
			ret=xcat(ret,xstr(map_id(map_val(ast,"space"),0), "if", map_id(map_val(ast,"space"),1), "(", map_id(map_val(ast,"space"),2), ast_c(map_id(map_val(ast,"left"),0)), map_id(map_val(ast,"space"),3), ")", map_id(map_val(ast,"space"),4), "{", map_id(map_val(ast,"space"),5), ast_c(map_id(map_val(ast,"right"),0)), map_id(map_val(ast,"space"),6), "}", End), End); };
	}else if(is_word(map_val(ast,"tag"),"if while for switch")){
		return xstr(map_id(map_val(ast,"space"),0), map_val(ast,"tag"), map_id(map_val(ast,"space"),1), "(", map_id(map_val(ast,"space"),2), ast_c(map_val(ast,"left")), map_id(map_val(ast,"space"),3), ")", map_id(map_val(ast,"space"),4), "{", map_id(map_val(ast,"space"),5), ast_c(map_val(ast,"right")), map_id(map_val(ast,"space"),6), "}", End);
	}else if(str_eq(map_val(ast,"tag"),";") && (!map_val(ast,"left") || str_eq(map_val(map_val(ast,"left"),"tag"),"TSpace"))){
		return map_val(map_val(ast,"left"),"space"); };
	return xstr(ast_c(map_val(ast,"left")), map_val(ast,"space"), map_val(ast,"tag"), ast_c(map_val(ast,"right")), map_val(ast,"tail_space"), End);
};
map* block_ast(map* toks,char* seperator){
	map* ret=new_vec();
	for(int idx=0; idx<toks->len; idx+=2){
		if(is_map(map_id(toks,idx+1))){
			vec_merge(ret,block_ast(map_id(toks,idx+1),seperator));
			continue; };
		map* temp=toks_ast(toks,&idx,seperator);
		vec_add(ret,xmap("tag", seperator, "left", temp, End));
		if(map_id(toks,idx)){ add(add_id(ret,map_len(ret)-1),"space",map_id(toks,idx)); }; };
	return ret;
};
map* toks_ast(map* toks,int* idx,char* upto){
	...
	char* operators="++ -- -> . ~ ! * / % + - << >> < <= > >= == === .= .. !== != & ^ @ # $ | && || ? : = += -= *= /= %= <<= >>= &= ^= |= \\ => ** *** **** ^^ ... , ] ) } ;";
	map* ret=NULL;
	int upto_no=is_word(upto,operators);
	while(*idx < toks->len){
		char* space=map_id(toks,*idx);
		char* v=map_id(toks,*idx+1);
		assert(!v || is_str(v));
		if(!v){
			if(space){
				if(ret){ add(ret,"tail_space",space); }
				else {ret=xmap("tag", "TSpace", "space", space, End);}; };
		}else if(is_numeric(v)){
			ret=xmap("tag", "TNumber", "right", v, "left", ret, End);
		}else if(str_eq(v,"if")){
		// if:1 (:3 __:5 ):7 {:9 __:11 }:13
		// else:15 if:17 (:19 __:21 ):23 {:25 __:27 }:29
		// else:31 if:33 (:35 __:37 ):39 {:41 __:43 }:45
		// else:47 {:49 __:51 }:53
			ret=xmap("tag", "if", "left", xvec(toks_ast(map_id(toks,*idx+5),NULL,";"), End), "right", xvec(block_ast(map_id(toks,*idx+11),";"), End), End);
			for(int i2=0; i2<14; i2+=2){ vec_add(add_key(ret,"space",Vector),map_id(toks,*idx+i2)); };
			*idx+=12;
			while(str_eq(map_id(toks,*idx+3),"else")){
				*idx+=4;
				if(str_eq(map_id(toks,*idx+1),"if")){
					vec_add(add_key(ret,"left",Vector),toks_ast(map_id(toks,*idx+5),NULL,";"));
					vec_add(add_key(ret,"right",Vector),block_ast(map_id(toks,*idx+11),";"));
					*idx+=12;
				}else{
					vec_add(add_key(ret,"right",Vector),block_ast(map_id(toks,*idx+3),";"));
					*idx+=6; }; };
		}else if(is_word(v,"while for switch")){
		// do:1 {:3 __:5 }:7 while:9 (:11 __:13 ):15
		// if:1 (:3 __:5 ):7 {:9 __:11 }:13
		// else:15 if:17 (:19 __:21 ):23 {:25 __:27 }:29
		// else:31 if:33 (:35 __:37 ):39 {:41 __:43 }:45
		// else:47 {:49 __:51 }:53
			ret=xmap("tag", v, "left", toks_ast(map_id(toks,*idx+5),NULL,";"), "right", block_ast(map_id(toks,*idx+11),";"), End);
			for(int i2=0; i2<14; i2+=2){ vec_add(add_key(ret,"space",Vector),map_id(toks,*idx+i2)); };
			*idx+=12;
		}else if(is_code(v)){
			ret=xmap("tag", "TName", "right", v, "left", ret, End);
		}else if(is_word(v,"[ ( {")){
			if(str_eq(v,"{")){
				ret=xmap("tag", v, "left", ret, "right", block_ast(map_id(toks,*idx+3),";"), End);
			}else if(str_eq(v,"(")){
				ret=xmap("tag", v, "left", ret, "right", toks_ast(map_id(toks,*idx+3),NULL,";"), End);
			}else if(str_eq(v,"[")){
				ret=xmap("tag", v, "left", ret, "right", block_ast(map_id(toks,*idx+3),NULL), End); };
			for(int i2=0; i2<6; i2+=2){ vec_add(add_key(ret,"space",Vector),map_id(toks,*idx+i2)); };
			*idx+=4;
		}else if(*v=='"'){
			ret=xmap("tag", "TString", "left", ret, "right", v, End);
		}else if(*v=='\''){
			ret=xmap("tag", "TChar", "left", ret, "right", v, End);
		}else if(upto_no && is_word(v,operators)>=upto_no){
			return ret;
		}else if(is_word(v,operators)){
			*idx+=2;
			ret=xmap("tag", v, "left", ret, "right", toks_ast(toks,idx,v), End);
			*idx-=2;
		}else {fox_error(xstr("fox_error ", v, End),0);};
		if(!map_val(ret,"space") && space){ add(ret,"space",space); };
		*idx+=2; };
	return ret;
};
...
char* syn_is_call(map* syn, int idx){
	if(is_code(map_id(syn,idx+1)) && str_eq(map_id(syn,idx+3),"(") && str_eq(map_id(syn,idx+7),")")){
...
char* file_foxh(char* infile,char* outfile){
	return write_file(funcs_cdecl(file_funcs(infile),1),outfile,0);
};
...
void* call_func(map* params,char* name,map* env){
	...
	if(str_start(name,"php_")){ return call_php(params,sub_str(name,5,0)); };
...
map* cmdline_params(map* args,char* func){
	...
	for(int i=next(args,-1,NULL,NULL); has_id(args,i); i++){ void* v=map_id(args,i); char* k=map_key(args, i);
		...
		if(str_start(v,"-") && str_len(v)>1){
			...
			if(str_start(v,"--")){
				named_param=sub_str(v,2,0);
				if(str_has(v,"=")){
					map* mp1=str_split(named_param,"=",2);
					named_param=map_id(mp1,0);
					val=map_id(mp1,1);
				}else {named_param=sub_str(v,2,0);};
			}else{
				named_param=sub_str(v,1,0);
...
int utests(char* test,char* file){
	...
	int passed=map_len(mp)/3+errs;
	px(xstr("\n",int_str( passed), "/",int_str( map_len(mp)/3), " tests passed.", End),1);
	return errs;
};
...
char* tutorial(){
	return ""
	"# Fox Language\n"
	"Fox language. Transcompiles souce into into human readable C. Maintaining your original code format.\n"
	"\n"
	"## Features\n"
	"\n"
	"### Small code base\n"
	"1K lines for the runtime. The compiler is 4K lines. Creates small 50K binary for hello world.\n"
	"\n"
	"### GC Collected\n"
	...
	"	increase(7)\n"
	"	increase(7,2)\n"
	"	xvec(7,2).call_func(:increase)\n"
	"	xvec(7).call_func(:increase)\n"
	"	xmap(NULL,:increase,:num,7,:inc_by,2).data_exec()\n"
	"	\"\n"
	"		increase\n"
	"		num=7\n"
	"		inc_by=2\n"
...
map* source_files(){ return xvec("core.fox","fox.fox","sql.fox","cmd.fox", End); };
map* source_funcs(){
	if(!map_val(map_val(_globals,"cache"),"funcs")){
		...
		map* map_1=source_files(); for(int i=next(map_1,-1,NULL,NULL); has_id(map_1,i); i++){ void* v=map_id(map_1,i);
			map* mp3=file_funcs(v);
			map_merge(mp,mp3); };
		add(add_key(_globals,"cache",Map),"funcs",mp); };
	return map_val(map_val(_globals,"cache"),"funcs");
};
map* file_funcs(char* filename){ return x_funcs(fox_read_file(filename,1),0); };
...
char* today(){ return substr(time_str(0),0,10); };
char* now(){ return substr(time_str(0),11,8); };
char* datetime(){ return time_str(0); };
void test_loop(){
	map* mp=xvec("one","two","three", End);
	for(int next1=next(mp, -1,NULL,NULL);has_id(mp,next1);next1+=1){
		char* val=map_id(mp,next1);
		px(val,1);	
	};
};
int crash(){
